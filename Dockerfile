...
RUN set -x \
	&& apt-get update && apt-get install -y --no-install-recommends ca-certificates wget jq php5 php5-mysqlnd && rm -rf /var/lib/apt/lists/* \
...
# percona-xtrabackup is installed at the same time so that `mysql-common` is only installed once from just mariadb repos
		...
		socat \
	&& rm -rf /var/lib/apt/lists/* \
# comment out any "user" entires in the MySQL config ("run.sh" or "--user" will handle user switching)
